...
with st.sidebar:
    ...
    st.markdown("<br><hr style='margin:0; border-color: rgba(255,255,255,0.1);'><p style='text-align:center; color:rgba(255,255,255,0.4); font-size:12px; margin-top:5px;'>💡 PharmSuk v48</p>", unsafe_allow_html=True)
        ...
        for r in pending_my_reqs:
            with st.container(border=True):
                ...
                if cols[2].button("🗑️ ยกเลิกคำขอ", key=f"del_pen_{r['id']}", use_container_width=True):
                    update_request_status(r['id'], "🗑️ ถูกยกเลิก"); st.rerun()
...
# ==================================================================
        ...
        for req in pending_reqs:
            with st.container(border=True):
                ...
                if c1.button("✅ อนุมัติ", key=f"app_{req['id']}", use_container_width=True):
                    update_request_status(req['id'], "✅ อนุมัติแล้ว"); st.rerun()
                if c2.button("❌ ปฏิเสธ", key=f"rej_{req['id']}", use_container_width=True):
                    update_request_status(req['id'], "❌ ไม่อนุมัติ"); st.rerun()
    ...
    if history_reqs:
        ...
        with pd.ExcelWriter(output_hist, engine='openpyxl') as writer: filtered_df.drop(columns=['ID']).to_excel(writer, index=False, sheet_name='History')
        ...
        with col_a2.popover("🗑️ จัดการฐานข้อมูล (Admin / Head)"):
            ...
            if st.button("ยืนยันการลบข้อมูลเก่า", type="primary"):
                ...
                st.toast(f"✅ ลบข้อมูลเก่าสำเร็จ {len(to_delete)} รายการ!"); time.sleep(1); st.rerun()
    ...
    if current_view == "ผู้ช่วยเภสัชกร":
        ...
        with col_t2:
            ...
            if st.button("🔄 ดึงข้อมูลล่าสุดจากปฏิทิน", use_container_width=True):
                ...
                st.toast("ดึงข้อมูลล่าสุดเรียบร้อย! 🔄"); st.rerun()
        ...
        with tab_l:
            for idx, l in enumerate(st.session_state.dash_leaves):
                ...
                if c2.button("❌ ลบ", key=f"d_l_{idx}"): st.session_state.dash_leaves.pop(idx); st.rerun()
        ...
        with tab_pt:
            for idx, pt in enumerate(st.session_state.dash_pts):
                ...
                if c2.button("❌ ลบ", key=f"d_p_{idx}"): st.session_state.dash_pts.pop(idx); st.rerun()
        ...
        with tab_t:
            for idx, t in enumerate(st.session_state.dash_tasks):
                ...
                if c2.button("❌ ลบ", key=f"d_t_{idx}"): st.session_state.dash_tasks.pop(idx); st.rerun()
        ...
        with tab_bo:
            for idx, bo in enumerate(st.session_state.dash_bo):
                ...
                if sc3.button("❌ ลบ", key=f"del_bo_{idx}"): st.session_state.dash_bo.pop(idx); st.rerun()
        ...
        with tab_sh:
            for idx, sh in enumerate(st.session_state.dash_shifts):
                ...
                if c3.button("❌ ลบ", key=f"d_sh_{idx}", use_container_width=True): st.session_state.dash_shifts.pop(idx); st.rerun()
        ...
        with tab_sub:
            ...
            if st.session_state.dash_subs:
                for idx, s in enumerate(st.session_state.dash_subs):
                    ...
                    if c2.button("❌ ลบ", key=f"d_s_{idx}"): st.session_state.dash_subs.pop(idx); st.rerun()
        ...
        with tab_lock:
            for idx, l in enumerate(st.session_state.dash_locks):
                c1, c2, c3 = st.columns([3, 4, 3])
                if l['type'] == 'task': c1.info(f"🔒 {l['user_name']}"); c2.info(f"หน้าที่: {l['task_name']} ({l['start']}-{l['end']} น.)")
                elif l['type'] == 'break': c1.success(f"☕ {l['user_name']}"); c2.success(f"เวลาพัก: ({l['start']}-{l['end']} น.)")
                elif l['type'] == 'no_dispense': c1.error(f"🚫 {l['user_name']}"); c2.error("เว้นการจ่ายยา (ทั้งวัน)")
                if c3.button("❌ ลบ", key=f"del_l_{idx}"): st.session_state.dash_locks.pop(idx); st.rerun()
        ...
        if st.session_state.gen_df is not None:
            ...
            if col_dl2.button("💾 บันทึกตารางลง Database", use_container_width=True):
                # 1. แปลงตาราง Pandas เป็นก้อนข้อมูล JSON
                raw_json = st.session_state.gen_df.to_json(orient="records", force_ascii=False)
                
                # 2. ส่งทั้ง HTML และ JSON ไปบันทึก
                if save_schedule_to_db(target_date_str, st.session_state.gen_html, raw_json): 
