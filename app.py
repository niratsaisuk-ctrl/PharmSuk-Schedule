...
st.markdown("""
    ...
    /* ซ่อนจุดวงกลมของ Radio Button และทำเมนูให้เป็นปุ่มกด */
...
# 💥 State สำหรับเก็บตาราง (กันตารางหายวับ)
    ...
    if st.session_state.gen_df is not None:
        ...
        <head>
            ...
            <style>
                ...
                .btn {{ width: 100%; background-color: #f0f2f6; color: #31333F; padding: 0.5rem 1rem; border: 1px solid rgba(49, 51, 63, 0.2); border-radius: 0.5rem; cursor: pointer; font-size: 16px; font-weight: 400; display: block; margin-bottom: 15px; font-family: 'Sarabun'; transition: all 0.3s ease;}}
                .btn:hover {{ border-color: #9B59B6; color: #9B59B6; transform: translateY(-2px); box-shadow: 0 4px 6px rgba(0,0,0,0.1);}}
                #capture-area-wrapper {{ position: relative; width: 100%; }}
            </style>
        </head>
        <body>
            <button class="btn" onclick="takeShot()" id="dl-btn">🖼️ บันทึกเป็นรูปภาพ (PNG)</button>
            ...
            <script>
                function takeShot() {{
                    ...
                    html2canvas(target, {{ scale: 2, useCORS: true, backgroundColor: '#ffffff' }}).then(canvas => {{
                        ...
                        link.click();
                        btn.innerText = '🖼️ บันทึกเป็นรูปภาพ (PNG)';
                    }});
                }}
            </script>
        </body>
