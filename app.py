...
with st.sidebar:
    ...
    # สวิตช์สลับหน้าต่าง (เห็นเฉพาะ Admin/Head)
    ...
    st.markdown("<br><hr style='margin:0; border-color: rgba(255,255,255,0.1);'><p style='text-align:center; color:rgba(255,255,255,0.4); font-size:12px; margin-top:5px;'>💡 PharmSuk v46</p>", unsafe_allow_html=True)

# ------------------------------------------------------------------
# เตรียมตัวแปร Global & ระบบคิวยกเลิกอัตโนมัติ (Queue Management)
# ------------------------------------------------------------------
...
# คำนวณคิวใหม่เสมอ! (ข้ามรายการที่ถูกยกเลิกหรือไม่อนุมัติ เพื่อให้คิวคนอื่นเลื่อนขึ้น)
leave_queues = {}
leaves_by_date = {}
...
def get_color_style(val):
    ...
    if '/' in val_str and '-' in val_str and val_str and val_str[0].isdigit(): return base + ' color: black; font-weight: bold;' 
    elif 'Match' in val_str and ('C' in val_str or 'C2' in val_str): return base + ' color: red; font-weight: bold;' 
    elif val_str == 'ลา': return base + ' color: black; font-weight: normal;'
    elif val_str in ['-', 'ว่าง', 'นอกเวลา']: return base + ' color: #808080;' 
...
with st.sidebar:
    ...
    # --- ย้ายปุ่มออกจากระบบมาไว้ใต้ชื่อทันที ---
    st.button("🚪 ออกจากระบบ", on_click=logout, use_container_width=True)
...
raw_requests = fetch_requests()
# กรองข้อมูลสำหรับมุมมองที่เลือก (เภสัชกร/ผู้ช่วย)
all_requests = [r for r in raw_requests if is_req_in_view(r)]

# ==================================================================
# คำนวณคิวใหม่เสมอ! (นับรวมทุกกลุ่มงาน ไม่แยกมุมมอง เพื่อความเป็นธรรม)
# ข้ามรายการที่ถูก "ยกเลิก" หรือ "ไม่อนุมัติ" เพื่อให้คิวคนอื่นเลื่อนขึ้นมาแทนที่
# ==================================================================
leave_queues = {}
leaves_by_date = {}
active_leaves_for_queue = [
...
if page == "🗓️ ปฏิทินห้องยา":
    ...
    # ให้ลางานมี Priority เริ่มต้นที่ 100 และบวกด้วยเลขคิว (ยิ่งน้อยยิ่งขึ้นบน)
    ...
    # เรียงลำดับ Requests ล่วงหน้าตาม Priority ก่อนเอาไปใส่ใน Calendar
...
# ==================================================================
    ...
    with tab_history:
        ...
        # ใช้ raw_requests เพื่อไม่ให้ประวัติโดนกรองหายไปเวลาสลับมุมมอง
...
# ==================================================================
    ...
    # ------------------ ส่วนตารางของผู้ช่วยเภสัชกร (ยังไม่พัฒนา) ------------------
    if current_view == "ผู้ช่วยเภสัชกร":
        st.info("🚧 **ระบบจัดตารางทำงานของ ผู้ช่วยเภสัชกร กำลังอยู่ในระหว่างการพัฒนา...**")
        st.write("ในอนาคต หน้านี้จะใช้สำหรับจัดเวร/ภาระงานเฉพาะของผู้ช่วยเภสัชกร (เช่น จัดยา, เติมยา, รับใบสั่งยา) โดยมีเงื่อนไขและ AI ที่ถูกออกแบบมาแยกต่างหากครับ")
        
    # ------------------ ส่วนตารางของเภสัชกร (ของเดิม) ------------------
    else:
        ...
        allow_head_assist = st.checkbox("🚨 โหมดฉุกเฉิน: อนุญาตให้ AI ดึงหัวหน้าลงมาช่วยงานหลักอัตโนมัติเมื่อคนไม่พอ", value=False, help="ระบบจะดึงหัวหน้ามาช่วย 'เฉพาะเวลาที่จำเป็นจริงๆ' เพื่อแก้ปัญหา Infeasible")
        ...
        if st.button("🚀 ประมวลผลสมองกล AI สร้างตาราง", type="primary", use_container_width=True):
            with st.spinner("🤖 AI กำลังคำนวณตำแหน่งและบล็อกเวลา..."):
                ...
                df_schedule, status, msg = generate_schedule(
                    ...
                    IS_MWF,
                    HEAD_PHARMACISTS=head_pharmacist_list,
                    ALLOW_HEAD_ASSIST=allow_head_assist 
                )
...
# ==================================================================
    ...
    for i, p in enumerate(st.session_state.pt_daily_db):
        p['_original_idx'] = i
            ...
            for pt in upcoming_pts:
                with st.container(border=True):
                    ...
                    if c2.button("🗑️ ลบข้อมูล", key=f"del_pt_up_{pt['_original_idx']}", use_container_width=True):
                        st.session_state.pt_daily_db.pop(pt['_original_idx'])
                        st.rerun()
    with t2:
        if not past_pts:
