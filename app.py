...
def generate_excel_bytes(df_schedule, target_date_str, DAY_OF_WEEK):
    target_dt = datetime.strptime(target_date_str, "%Y-%m-%d").date()
    output = io.BytesIO()
...
# ==================================================================
    ...
    if current_view == "ผู้ช่วยเภสัชกร":
        ...
        # ตรวจสอบว่ามีตารางของวันนี้ในระบบหรือยัง
        existing_sched = None
        if supabase:
            res_ex = supabase.table("schedules").select("*").eq("schedule_date", target_date_str).execute()
            if len(res_ex.data) > 0:
                existing_sched = res_ex.data[0]
        ...
        # ---------------- ส่วนดึงข้อมูลตารางเดิม ----------------
        if existing_sched and existing_sched.get('raw_data'):
            last_edit = existing_sched.get('created_at', '')[:16].replace('T', ' ')
            st.info(f"📅 พบตารางของวันที่ {target_date_str} ถูกบันทึกไว้ในระบบแล้ว (แก้ไขล่าสุด: {last_edit})")
            if st.button("📂 โหลดตารางเดิมขึ้นมาแก้ไขด่วน", use_container_width=True):
                raw_json = existing_sched['raw_data']
        ...
        # ---------------- ส่วนแสดง Data Editor ให้คลิกแก้ไขได้ ----------------
        if st.session_state.gen_df is not None:
            if st.session_state.show_balloons:
                st.balloons()
                st.success("🎉 AI คำนวณตารางเสร็จสมบูรณ์! (ปรับแก้ช่องตารางด้านล่างนี้ได้เลยก่อนบันทึก)")
                st.session_state.show_balloons = False
            
            st.markdown("### ✏️ ปรับแก้ตารางฉุกเฉินด้วยมือ (Manual Override)")
            st.caption("💡 ดับเบิลคลิกที่เซลล์ในตารางเพื่อแก้ไขภาระงาน หรือเปลี่ยนตัวบุคคลได้เลย เมื่อแก้เสร็จให้กดปุ่มบันทึกด้านล่าง")
            
            # กางตาราง Interactive ให้คลิกแก้ได้ทันที
            edited_df = st.data_editor(st.session_state.gen_df, use_container_width=True, hide_index=True)
            
            # นำตารางที่ผ่านการแก้ไข ไปแปลงเป็น HTML และ Excel เตรียมไว้
            new_html = build_html_table(edited_df, target_date_str, DAY_OF_WEEK)
            excel_bytes = generate_excel_bytes(edited_df, target_date_str, DAY_OF_WEEK)
            
            st.markdown("### 📥 บันทึกผลลัพธ์")
            col_dl1, col_dl2 = st.columns(2)
            col_dl1.download_button(label="📊 ดาวน์โหลดไฟล์ Excel", data=excel_bytes, file_name=f"Schedule_{target_date_str}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
            
            if col_dl2.button("💾 บันทึก/อัปเดตตารางลง Database", use_container_width=True):
                # เอาตารางที่แก้แล้วแปลงเป็น JSON ส่งเข้าฐานข้อมูล
                ...
                if save_schedule_to_db(target_date_str, new_html, raw_json): 
                    ...
                    st.error("❌ บันทึกล้มเหลว (โปรดตรวจสอบการตั้งค่าฐานข้อมูล)")
            ...
            <div id="capture-area-wrapper">{new_html}</div>
            <script>
                function takeShot() {{
