...
with st.sidebar:
    ...
    st.markdown("<br><hr style='margin:0; border-color: rgba(255,255,255,0.1);'><p style='text-align:center; color:rgba(255,255,255,0.4); font-size:12px; margin-top:5px;'>💡 PharmSuk v50</p>", unsafe_allow_html=True)
...
# ==================================================================
    ...
    if current_view == "ผู้ช่วยเภสัชกร":
        ...
        # เพิ่มการตรวจสอบกรณีวันที่เปลี่ยน เพื่อล้างความจำตารางที่ค้างอยู่ (แก้บั๊กของเก่า)
        ...
        # --- ตรวจสอบว่ามีข้อมูลเก่าในระบบไหม เพื่อทำปุ่มโหลดตาราง ---
                        ...
                        with pd.ExcelWriter(output, engine='openpyxl') as writer:
                            ...
                            for col_idx in range(1, len(df_schedule.columns) + 1):
                                ...
                                for row_idx in range(3, len(df_schedule) + 4): 
                                    ...
                                    if col_idx == 1 or row_idx == len(df_schedule) + 3: cell.font = Font(name='TH Sarabun New', size=16, bold=True)
        ...
        with col_load:
            if saved_data and saved_data.get('raw_data'):
                if st.button("📂 โหลดตารางเดิมขึ้นมาแก้ไขด่วน", use_container_width=True):
                    ...
                    # รีบิลด์ไฟล์ Excel จากตารางเดิม
                    output = io.BytesIO()
                    with pd.ExcelWriter(output, engine='openpyxl') as writer:
                        st.session_state.gen_df.to_excel(writer, index=False, sheet_name='ตารางเวร', startrow=2)
                        # (ตั้งค่าฟอร์แมต Excel คร่าวๆ เพื่อให้ดาวน์โหลดได้)
                    st.session_state.gen_excel = output.getvalue()
        ...
        if st.session_state.gen_df is not None:
            ...
            # แทนที่การโชว์ตารางเฉยๆ ด้วย data_editor ให้หัวหน้าแก้ตารางได้
            ...
            if col_dl2.button("💾 บันทึกตารางลง Database", use_container_width=True):
                # ก่อนเซฟ เราต้องเอา edited_df มาวาด HTML และ JSON ใหม่ เพื่อรับค่าที่เพิ่งถูกแก้ไขไป
