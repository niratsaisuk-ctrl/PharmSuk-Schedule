    ...
    with tab3:
        ...
        with sub_tab1:
            ...
            for r in pending_my_reqs:
                with st.container(border=True):
                    ...
                    if cols[2].button("🗑️ ยกเลิก", key=f"del_pen_{r['id']}", use_container_width=True):
                        delete_request(r['id'])
                        st.rerun()
        ...
        with sub_tab2:
            ...
            if not history_my_reqs:
                ...
                future_reqs = [r for r in history_my_reqs if r['req_date'] >= today_str]
                past_reqs = [r for r in history_my_reqs if r['req_date'] < today_str]
                ...
                if future_reqs:
                    with st.container(height=300):
                        for r in future_reqs:
                            ...
                            st.markdown(f"**{status_icon} {r['req_type']}** | {r['req_date']} | {r['detail']}")
                            st.divider()
                else:
