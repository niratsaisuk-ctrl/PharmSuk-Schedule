...
st.markdown("""
    ...
    /* 🌟 เปลี่ยนสีเมนูที่เลือกอยู่ (Active State) */
...
dispensing_tasks = [f"จ่ายยา_{i}" for i in range(4, 12)]
ver_cpoe_tasks = [f"Ver_{i}" for i in range(1, 11)]
ver_ps_tasks = [f"PS_{i}" for i in range(1, 11)]
base_main_tasks = dispensing_tasks + ver_cpoe_tasks + ver_ps_tasks + ["Match_C", "Match_C2"]
...
def generate_schedule(DAY_OF_WEEK, LEAVES, CUSTOM_TASKS, PART_TIME, FIX_BREAKS, FIXED_MAIN_TASKS, SICK_PEOPLE, IS_MWF):
    ...
    # 💥 แก้ไขจุดที่เกิด Error: เปลี่ยน f'Ver {i}' เป็น f'Ver_{i}' และ f'PS_{i}' ให้ตรงกับชื่อ Task ใน Model
    for t in range(16):
