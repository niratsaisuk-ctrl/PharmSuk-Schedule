...
# ------------------------------------------------------------------
# เตรียมตัวแปร Global ที่แปรผันตามมุมมอง (Context) ที่เลือก
# ------------------------------------------------------------------
current_view = st.session_state.current_view
users_db = fetch_users()

# ใช้สำหรับ Map ชื่อ full_name -> ตำแหน่ง เพื่อกรองคำขอ (Requests)
fullname_to_pos = {u['full_name']: get_std_pos(u) for u in users_db.values()}

# กรองเฉพาะ Active Users ของมุมมองที่เลือกอยู่ (และจัดเรียง)
...
# ฟังก์ชันกรองคำขอเฉพาะมุมมองที่เลือก
...
def force_sync_dashboard(target_date_str, all_requests):
    ...
    # กรองพาร์ทไทม์เฉพาะของมุมมองปัจจุบัน
...
# ------------------------------------------------------------------
# 4. สมองกล AI V137 ต้นฉบับ 100% (สำหรับเภสัชกร)
# ------------------------------------------------------------------
        ...
        for t in range(16):
            if t not in fixed_slots:
                for task in dispensing_tasks + ver_cpoe_tasks + ver_ps_tasks + ['Match + C', 'Match + C2']:
                    if ALLOW_HEAD_ASSIST:
                        reward_vars.append(x[p, t, task] * -5000000)
                    else:
...
if page == "🗓️ ปฏิทินห้องยา":
    ...
    if filter_option == "แสดงทั้งหมด":
        for pt in st.session_state.pt_daily_db:
            # กรองให้แสดงเฉพาะ PT ของมุมมองปัจจุบัน
...
# ==================================================================
    ...
    # ดึงข้อมูลคำขอแล้วกรองเฉพาะมุมมองปัจจุบัน
    raw_requests = fetch_requests()
    all_requests = [r for r in raw_requests if is_req_in_view(r)]
    ...
    with tab_history:
        ...
        my_reqs = [r for r in all_requests if r["user_name"] == user_info['full_name'] or (user_info['role'] in ['Admin', 'Head'] and r["user_name"] == "SYSTEM_REQ")]
...
# ==================================================================
    ...
    st.subheader(f"📌 คำขอที่รอการอนุมัติ")
...
# ==================================================================
    ...
    # ------------------ ส่วนตารางของเภสัชกร (ของเดิม) ------------------
        ...
        all_requests = [r for r in fetch_requests() if is_req_in_view(r)]
        approved_today = [r for r in all_requests if r["req_date"] == target_date_str and r["status"] == "✅ อนุมัติแล้ว"]
        pts_today = [pt for pt in st.session_state.pt_daily_db if pt["date"] == target_date_str and pt.get('position', 'เภสัชกร') == current_view]
...
# ==================================================================
        ...
        if st.button("บันทึกพาร์ทไทม์ลงระบบ", type="primary", key="btn_pt_save_page"):
            if pt_name:
                st.session_state.pt_daily_db.append({
                    ...
                    "position": current_view # ฝังตำแหน่งไว้ที่มุมมองปัจจุบัน
                })
                st.success("✅ บันทึกข้อมูลสำเร็จ!")
                time.sleep(1)
                st.rerun()
    ...
    # กรองแสดงเฉพาะ PT ของตำแหน่งปัจจุบัน
...
# ==================================================================
    ...
    with st.form("add_user_form", clear_on_submit=True):
        ...
        # เปลี่ยนเป็น Selectbox เพื่อป้องกันการสะกดผิด
    ...
    for u in active_users:
        with st.container(border=True):
            ...
            with c2:
                ...
                # เพิ่มช่องให้สามารถย้ายตำแหน่งสลับไปมาได้
