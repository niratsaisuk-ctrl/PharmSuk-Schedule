...
# ------------------------------------------------------------------
# 4. สมองกล AI V137 ต้นฉบับ 100% + 💥 โหมดหัวหน้าช่วยด้วย (Auto-Assist)
# ------------------------------------------------------------------
...
# 💥 เพิ่มพารามิเตอร์ ALLOW_HEAD_ASSIST
def generate_schedule(DAY_OF_WEEK, LEAVES, CUSTOM_TASKS, PART_TIME, FIX_BREAKS, FIXED_MAIN_TASKS, SICK_PEOPLE, IS_MWF, ALLOW_HEAD_ASSIST=False):
    ...
    # 💥 กฎของหัวหน้าห้องยา + โหมด Auto-Assist
            ...
            if t not in fixed_slots:
                for task in dispensing_tasks + ver_cpoe_tasks + ver_ps_tasks + ['Match + C', 'Match + C2']:
                    if ALLOW_HEAD_ASSIST:
                        # 💥 อนุญาตให้หัวหน้าทำได้ แต่โดนทำโทษหนักมาก (-5 ล้าน) เพื่อให้ AI เลือกวิธีนี้เป็นทางออกสุดท้าย (Last Resort)
                        reward_vars.append(x[p, t, task] * -5000000)
                    else:
                        # 💥 แบบดั้งเดิม: บังคับห้ามหัวหน้าทำเด็ดขาด (x == 0)
                ...
                for tsk in tasks:
                    if solver.Value(x[p, t, tsk]) == 1:
                        if tsk == 'งานเฉพาะ': assigned = custom_dict_index.get((p, t), 'งานเฉพาะ')
                            ...
                            # 💥 ใส่คำว่า (ช่วย) ต่อท้ายภาระงาน ถ้าคนนั้นคือหัวหน้าและอยู่ในโหมด Auto-Assist
...
with st.sidebar:
    ...
    st.markdown("<br><hr style='margin:0; border-color: rgba(255,255,255,0.1);'><p style='text-align:center; color:rgba(255,255,255,0.4); font-size:12px; margin-top:5px;'>💡 PharmSuk v44</p>", unsafe_allow_html=True)

# ==================================================================
# หน้า 1.1: 🗓️ ปฏิทินห้องยา 
# ==================================================================
if page == "🗓️ ปฏิทินห้องยา":
    st.title("🗓️ ปฏิทินห้องยา")
...
# ==================================================================
    ...
    # 💥 สวิตช์เปิดปิด "โหมดหัวหน้าช่วยด้วย"
    allow_head_assist = st.checkbox("🚨 โหมดฉุกเฉิน: อนุญาตให้ AI ดึงหัวหน้า (กอล์ฟ/มุก) ลงมาช่วยงานหลักอัตโนมัติเมื่อคนไม่พอ", value=False, help="ระบบจะดึงหัวหน้ามาช่วย 'เฉพาะเวลาที่จำเป็นจริงๆ' เพื่อแก้ปัญหา Infeasible")
    ...
    if st.button("🚀 ประมวลผลสมองกล AI สร้างตาราง", type="primary", use_container_width=True):
        with st.spinner("🤖 AI กำลังคำนวณตำแหน่งและบล็อกเวลา..."):
            ...
            # 💥 ส่งพารามิเตอร์โหมดหัวหน้าช่วยด้วย เข้าไปในสมองกล AI
