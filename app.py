...
# ------------------------------------------------------------------
# 1. ตั้งค่าหน้าเว็บ & เวทมนตร์ CSS (Modern UI with Seamless Gradient)
# ------------------------------------------------------------------
st.set_page_config(page_title="PharmSuk App", layout="wide", page_icon="💊")

# ดึงสีจาก Banner: Teal, Purple, Light Blue มาทำ Gradient
st.markdown("""
    <style>
    ...
    /* 💥 เปลี่ยนพื้นหลังทั้งแอพให้เป็นสีพริ้วๆ โทนเดียวกับ Banner */
    .stApp {
        background: rgb(240,248,255);
        background: radial-gradient(circle at 10% 10%, rgba(128,222,234,0.15) 0%, rgba(240,248,255,1) 50%, rgba(225,190,231,0.15) 100%);
    }
    ...
    .stButton>button:hover {
        ...
        border-color: #4DD0E1 !important; /* สี Teal จาก Banner */
    ...
    div[data-testid="stExpander"] {
        ...
        background-color: rgba(255,255,255,0.7) !important; /* โปร่งแสงนิดๆ */
    ...
    .stTabs [aria-selected="true"] {
        background-color: rgba(128,222,234,0.2) !important; /* สี Teal อ่อน */
    ...
    /* 💥 เปลี่ยนสีแถบเมนูข้าง (Sidebar) ให้เข้มขึ้นนิดนึง แต่เป็นโทนพริ้วๆ เหมือนกัน */
...
# ------------------------------------------------------------------
def login_page():
    # 💥 ปรับหน้า Login ให้ Banner ลอยเด่น กลืนไปกับพื้นหลัง Gradient ใหม่
    col_img1, col_img2, col_img3 = st.columns([1, 2, 1])
    with col_img2:
        if os.path.exists("banner.png"):
            # ใช้ CSS เอาขอบขาวออก และทำให้ภาพดูลอย
            st.markdown(
                """
                <style>
    ...
    with col2:
        # กล่อง Login โปร่งแสง
        with st.form("login_form"):
            st.markdown("<h3 style='text-align: center; color: #154360; margin-top:0;'>เข้าสู่ระบบ</h3>", unsafe_allow_html=True)
...
with st.sidebar:
    # Banner ใน Sidebar มีเงาและขอบมนให้ดูมีมิติ
    if os.path.exists("banner.png"):
        st.image("banner.png", use_container_width=True)
...
if page == "🗓️ ปฏิทินห้องยา & ลงข้อมูล":
    ...
    with tab1:
        ...
        for h_date, h_name in th_holidays.items():
            events.append({"start": h_date.strftime("%Y-%m-%d"), "display": "background", "backgroundColor": "rgba(255,205,210,0.5)", "priority": 0})
    ...
    with tab_sh:
        ...
        with st.expander("➕ เพิ่มการออกเวรหน้างาน"):
            ...
            if add_sh_t == "ออกเวรดึก":
                ...
                with cl2: add_sh_e = st.selectbox("ถึงเวลา", VALID_TIMES, index=4)
            else:
                sc1, sc2 = st.columns(2)
    ...
    if st.session_state.gen_df is not None:
        ...
        <head>
            ...
            <style>
                ...
                .btn-img {{ width: 100%; background-color: rgba(255,255,255,0.8); color: #154360; padding: 0.5rem 1rem; border: 1px solid #4DD0E1; border-radius: 8px; cursor: pointer; font-size: 16px; font-weight: 500; font-family: 'Kanit', sans-serif; transition: all 0.3s ease; display: flex; align-items: center; justify-content: center; height: 42px; }}
                .btn-img:hover {{ border-color: #154360; color: white; background-color: #154360; transform: translateY(-2px); box-shadow: 0 4px 10px rgba(0,0,0,0.1); }}
                #capture-area-wrapper {{ position: relative; width: 100%; overflow-x: auto; padding-bottom: 10px; }}
                .btn-container {{ display: flex; justify-content: flex-start; padding-top: 10px; }}
            </style>
        </head>
        <body>
            <div id="capture-area-wrapper">{st.session_state.gen_html}</div>
            <div class="btn-container">
                <button class="btn-img" onclick="takeShot()" id="dl-btn">🖼️ บันทึกเป็นรูปภาพ (PNG)</button>
            </div>
            <script>
                function takeShot() {{
        ...
        components.html(full_html, height=730, scrolling=False)
        
        c1, c2, c3 = st.columns([1, 1, 1])
        c2.download_button(label="📥 ดาวน์โหลดไฟล์ Excel", data=st.session_state.gen_excel, file_name=f"Schedule_{target_date_str}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
...
# ==================================================================
    ...
    # สมมติรายชื่อผู้ใช้
    mock_users = [
        {"username": "admin", "full_name": "แอดมิน", "role": "Admin"},
        {"username": "pharma1", "full_name": "เภสัชกร 1", "role": "Staff"},
    ]
    for u in sorted(mock_users, key=lambda x: x['role']):
