            ...
            for t in range(16):
                ...
                for tsk in tasks:
                    if solver.Value(x[p, t, tsk]) == 1:
                        ...
                        # 💥 แก้ให้แสดงคำว่า 'ลา' ถ้าติดสถานะ 'ลา'
...
def get_cell_bg_hex(val_str):
    ...
    elif val == 'ลา': return "FFCDD2" # 💥 เพิ่มพื้นหลังสีแดงพาสเทลให้สถานะ 'ลา'
...
def build_html_table(df, selected_date_str, DAY_OF_WEEK):
    ...
    def get_cell_style(val_str):
        ...
        elif val_str == 'ลา': color, weight = "#C0392B", "bold" # 💥 ตัวอักษรสีแดงเข้มสำหรับคำว่าลา
...
if page == "🗓️ ปฏิทินห้องยา & ลงข้อมูล":
    ...
    # 💥 คิวการลา (Queue System) คำนวณจากเวลาส่ง Request
    leave_queues = {}
    leaves_by_date = {}
    ...
    with tab2:
        ...
        if user_info['role'] == 'Admin': 
            ...
            form_options.append("⚙️ กำหนดโควตาลา") # 💥 เมนูกำหนดโควตา
        ...
        if "ลางาน" in main_type:
            ...
            leave_remark = st.text_input("หมายเหตุ เช่น พาร์ทไทม์ที่มาแทน หรือ การมอบหมายงาน") # 💥 ช่องหมายเหตุใหม่
        ...
        elif "กำหนดโควตาลา" in main_type: # 💥 ระบบกำหนดโควตาลา
            req_date = st.date_input("วันที่:")
...
# ==================================================================
    ...
    # 💥 ดึงข้อมูลและโชว์คิวลาในหน้าอนุมัติ
    all_requests = fetch_requests()
    leave_queues = {}
    leaves_by_date = {}
    sorted_for_queue = sorted(all_requests, key=lambda x: x.get('created_at', ''))
...
# ==================================================================
    ...
    if st.button("🚀 ประมวลผลสมองกล AI สร้างตาราง", type="primary", use_container_width=True):
        with st.spinner("🤖 AI กำลังคำนวณตำแหน่งและบล็อกเวลา..."):
            ...
            # 💥 แปลง Tuple ของลาฉุกเฉินให้ AI เข้าใจอย่างถูกต้อง
            leaves_dict = {}
